// server.js - HARDCODED PATH (works now)
const express = require('express');
const app = express();
const PORT = process.env.PORT || 3000;

// Hardcoded secret path – change this later
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: -apple-system, sans-serif; background: #0a0e17; color: #e0e6ed; padding: 20px; }
        .container { max-width: 800px; margin: 0 auto; }
        .header { display: flex; justify-content: space-between; align-items: center; padding: 20px 0; border-bottom: 1px solid #1a2332; margin-bottom: 20px; }
        ...
        .stat-card { background: #111927; border: 1px solid #1a2332; border-radius: 10px; padding: 15px; text-align: center; }
        .stat-card .value { font-size: 28px; font-weight: 700; color: #4fc3f7; }
        .stat-card .label { font-size: 12px; color: #8896ab; }
        .section { background: #111927; border: 1px solid #1a2332; border-radius: 10px; padding: 20px; margin-bottom: 20px; }
        ...
        .admin-gry { position: fixed; bottom: 10px; right: 10px; font-size: 8px; color: rgba(79,195,247,0.05); user-select: none; pointer-events: none; }
        .hidden { display: none; }
        .login-container { max-width: 350px; margin: 80px auto; background: #111927; border: 1px solid #1a2332; border-radius: 16px; padding: 40px; text-align: center; }
        .login-container input { width: 100%; padding: 12px; margin: 8px 0; background: #0a0e17; border: 1px solid #1a2332; border-radius: 8px; color: #e0e6ed; font-size: 14px; }
        .login-container input:focus { outline: none; border-color: #4fc3f7; }
        .login-container button { width: 100%; padding: 12px; margin-top: 10px; background: #4fc3f7; border: none; border-radius: 8px; color: #0a0e17; font-size: 16px; font-weight: 600; cursor: pointer; }
    </style>
</head>
<body>
...
app.get('/a9f3k217/api/numbers', (req, res) => res.json([]));

// 404 for everything else (no catch-all debug)
app.use((req, res) => {
    res.status(404).send(`
        <html>
